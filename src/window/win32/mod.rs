...
use windows_sys::Win32::{
	...
	UI::WindowsAndMessaging::{
		AdjustWindowRectEx, CREATESTRUCTW, DefWindowProcW, DestroyWindow,
		DispatchMessageW, MSG, PM_REMOVE, PeekMessageW, SW_SHOW, SWP_NOMOVE,
		SWP_NOZORDER, SetWindowPos, SetWindowTextW, ShowWindow, TranslateMessage,
		WM_CLOSE, WM_CREATE, WM_LBUTTONDOWN, WM_LBUTTONUP, WM_MBUTTONDOWN, WM_MBUTTONUP,
		WM_MOUSEMOVE, WM_NCCREATE, WM_RBUTTONDOWN, WM_RBUTTONUP, WM_SIZE, WNDCLASSEXW,
		WNDCLASSW, WS_OVERLAPPEDWINDOW,
	},
};

use crate::window::{Input, MouseButton, MouseState, WindowOptions};
...
impl WindowServer {
	pub fn connect() -> (Self, WindowClient) {
		...
		let res = mpsc::channel::<Response>();
		let req = mpsc::channel::<Request>();
	...
	pub fn run(&mut self) {
		// É necessário criar uma janela invisível para receber os eventos do windows.
		unsafe {
	...
	fn process_requests(&mut self) {
		if let Ok(req) = self.req.try_recv() {
			match req {
				Request::CreateWindow { options } => {
					let window = self.create_window(options);
					self.res.send(Response::WindowCreated { window }).unwrap();
				}
	...
	unsafe extern "system" fn on_event(
		...
		match msg {
			...
			WM_MOUSEMOVE => {
				ctx.send_event(Input::MouseMove {
					x: (lp & 0xFFFF) as i32,
					y: ((lp >> 16) & 0xFFFF) as i32,
				});
				return 0;
...
impl WindowClient {
	pub fn new_window(&self, options: WindowOptions) -> Window {
		self.req
			.send(Request::CreateWindow { options })
		...
		if let Response::WindowCreated { window } = res {
			return window;
		} else {
	...
	pub fn poll_events<H: FnMut(Input)>(&self, mut handler: H) {
		let mut events = self.events.lock().unwrap();

		if let Some(Input::Resize { .. }) = &events.resize {
			events.resize.take().map(|ev| handler(ev));
		}
	...
	pub fn terminate(&self) {
		self.req.send(Request::Terminate);
	}
}
...
impl RunCtx {
	...
	fn send_event(&mut self, event: Input) {
		let mut events = self.events.lock().unwrap();
